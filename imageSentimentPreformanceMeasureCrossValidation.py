...
for trainIndx, testIndx in statifiedFolds.split(allImsPaths, labels):
    ...
    yTrain_pred= model.predict_classes(x_trainImSet, verbose=0)
    yTest_pred= model.predict_classes(x_testImSet, verbose=0)
    #call the confusion matrix function to collect true positive, false positive, true negative and false negative 
    tnTrn, fpTrn, fnTrn, tpTrn = confusion_matrix(y_trainSetBinary, yTrain_pred).ravel()
    tnTst, fpTst, fnTst, tpTst = confusion_matrix(y_testSetBinary, yTest_pred).ravel()
    #store true positive, false positive, true negative and false negative in DF
    performanceTb=performanceTb.append({'FoldNum':foldi, 'TrnOrTest':'Train','TN':tnTrn, 'FP':fpTrn, 'FN':fnTrn, 'TP':tpTrn},ignore_index=True)
    performanceTb=performanceTb.append({'FoldNum':foldi, 'TrnOrTest':'Test','TN':tnTst, 'FP':fpTst, 'FN':fnTst, 'TP':tpTst},ignore_index=True)
    foldi=foldi+1
    
#Write the DF performace to a XLS file
performanceTb.to_csv('ConfusionCV.xls',index = None, header=True)
